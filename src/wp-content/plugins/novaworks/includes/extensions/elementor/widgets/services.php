 ...
 * Services Widget
 */
class Services extends NOVA_Widget_Base {

    public function get_name() {
        return 'novaworks-services';
    }

    protected function get_widget_title() {
        return esc_html__( 'Services', 'novaworks' );
    }

    public function get_style_depends() {
        return [
            'novaworks-services-elm'
        ];
    }

    public function get_icon() {
        return 'novaworkselements-icon-26';
    }

    protected function register_controls() {
        $css_scheme = apply_filters(
            'NovaworksElement/services/css-scheme',
            array(
                'instance'         => '.novaworks-services',
                'instance_inner'   => '.novaworks-services__inner',
                'header'           => '.novaworks-services__header',
                'cover'            => '.novaworks-services__cover',
                'figure'           => '.novaworks-services__figure',
                'content'          => '.novaworks-services__content',
                'icon'             => '.novaworks-services__icon',
                'title'            => '.novaworks-services__title',
                'title_icon'       => '.novaworks-services__title-icon',
                'title_text'       => '.novaworks-services__title-text',
                'desc'             => '.novaworks-services__desc',
                'button'           => '.novaworks-services__button',
                'button_icon'      => '.novaworks-services__button-icon',
            )
        );
        ...
        $this->add_control(
            'img_type',
            array(
                'label'   => esc_html__( 'Image Type', 'novaworks' ),
        ...
        $this->add_control(
            ...
            array(
                ...
                'default' => esc_html__( 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.', 'novaworks' ),
                'dynamic' => array( 'active' => true ),
            )
        );
        ...
        $this->add_control(
            ...
            array(
                ...
                'type'    => Controls_Manager::TEXT,
                'default' => esc_html__( 'More', 'novaworks' ),
            )
        );
        ...
        $this->add_control(
            'clickable_banner',
            array(
                'label'        => esc_html__( 'Clickable on all', 'novaworks' ),
        ...
        $this->add_control(
            'header_position',
            array(
                'label'   => esc_html__( 'Header Position', 'novaworks' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'top',
                'options' => array(
        ...
        $this->start_controls_section(
            ...
            array(
                'label'      => esc_html__( 'Icon & Image', 'novaworks' ),
                'tab'        => Controls_Manager::TAB_STYLE,
                'show_label' => false,
            )
        );
        ...
        $this->add_control(
            'icon_cover_location',
            array(
                'label'        => esc_html__( 'Display in header', 'novaworks' ),
                'type'         => Controls_Manager::SWITCHER,
                'label_on'     => esc_html__( 'Yes', 'novaworks' ),
                'label_off'    => esc_html__( 'No', 'novaworks' ),
                'return_value' => 'yes',
        ...
        $this->add_responsive_control(
            ...
            array(
                ...
                'selectors'  => array(
                    ...
                    '{{WRAPPER}} .novaworks-services__icon_img' => 'justify-content: {{VALUE}};',
                ),
            )
        );
        ...
        $this->add_control(
            'use_title_icon',
            array(
                'label'        => esc_html__( 'Use title icon?', 'novaworks' ),
                'type'         => Controls_Manager::SWITCHER,
                'label_on'     => esc_html__( 'Yes', 'novaworks' ),
                'label_off'    => esc_html__( 'No', 'novaworks' ),
                'return_value' => 'yes',
        ...
        $this->add_control(
            'title_icon',
            array(
                'label'       => esc_html__( 'Title Icon', 'novaworks' ),
        ...
        $this->add_responsive_control(
            'title_icon_margin',
            array(
                'label'      => __( 'Icon Margin', 'novaworks' ),
                'type'       => Controls_Manager::DIMENSIONS,
                'size_units' => array( 'px', '%' ),
                'selectors'  => array(
        ...
        $this->add_control(
            'button_fullwidth',
            array(
                'label'        => esc_html__( 'Enable Button Fullwidth', 'novaworks' ),
                'type'         => Controls_Manager::SWITCHER,
                'label_on'     => esc_html__( 'Yes', 'novaworks' ),
                'label_off'    => esc_html__( 'No', 'novaworks' ),
                'return_value' => 'yes',
    ...
    public function __get_services_image() {
        ...
        $service_image = sprintf( apply_filters('NovaworksElement/services/image-format', '<img src="%1$s" data-src="%2$s" alt="" loading="lazy" class="nova-lazyload-image %3$s" %4$s>'), $giflazy, $params[0], 'novaworks-banner__img' , $srcset);
        return sprintf( '<div class="novaworks-services__icon novaworks-services__icon_img"><div class="inner">%s</div></div>', $service_image );
    }
    ...
    public function __generate_icon( $cover_location = false ) {
        ...
        echo '<div class="novaworks-services__icon"><div class="inner">';
    ...
    public function __generate_title( $cover_location = false ) {
        ...
        if ( ! empty( $title_icon ) ) {
            $icon_html = sprintf( '<span class="novaworks-services__title-icon"><i class="%s"></i></span>', $title_icon );
        }

        if ( ! empty( $title ) ) {

            $title_html = sprintf( '<span class="novaworks-services__title-text">%s</span>', $title );
        }

        $title_tag = $this->get_settings_for_display( 'services_title_size' );

        $format = apply_filters( 'NovaworksElement/services/name-format', '<%3$s class="novaworks-services__title">%1$s%2$s</%3$s>' );
    ...
    public function __generate_description( $cover_location = false ) {
        ...
        $format = apply_filters( 'NovaworksElement/services/description-format', '<p class="novaworks-services__desc">%s</p>' );
    ...
    public function __generate_action_button( $cover_location = false ) {
        ...
        if ( filter_var( $use_icon, FILTER_VALIDATE_BOOLEAN ) ) {
            $icon_html = sprintf( '<i class="novaworks-services__button-icon %s"></i>', $button_icon );
        }

        $this->add_render_attribute( 'url', 'class', array(
            'elementor-button',
            'elementor-size-md',
            'novaworks-services__button',
            'novaworks-services__button--icon-' . $icon_position,
        ) );
        ...
        $format = apply_filters( 'NovaworksElement/services/action-button-format', '<a %1$s><span class="novaworks-services__button-text">%2$s</span>%3$s</a>' );
