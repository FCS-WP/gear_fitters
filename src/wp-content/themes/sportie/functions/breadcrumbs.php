...
function nova_breadcrumbs( $args = '' ) {
	$args = wp_parse_args( $args, array(
		...
		'show_on_front'     => false,
		'labels'            => array(
			'home'      => esc_html__( 'Home', 'sportie' ),
			'archive'   => esc_html__( 'Archives', 'sportie' ),
			'blog'      => esc_html__( 'Blog', 'sportie' ),
			'search'    => esc_html__( 'Search results for', 'sportie' ),
			'not_found' => esc_html__( 'Not Found', 'sportie' ),
			'author'    => esc_html__( 'Author Archives:', 'sportie' ),
			'day'       => esc_html__( 'Daily Archives:', 'sportie' ),
			'month'     => esc_html__( 'Monthly Archives:', 'sportie' ),
			'year'      => esc_html__( 'Yearly Archives:', 'sportie' ),
		),
	) );
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'sportie' ), $args['labels']['day'], get_the_date() )
		);
	} // Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'sportie' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	} // Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'sportie' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	} // Archive
	else {
		$items[] = sprintf(
